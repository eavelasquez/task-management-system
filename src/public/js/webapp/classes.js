...
export class TaskList {
  ...
  constructor() {
    if (TaskList.instance) {
      throw new Error("Use TaskList.getInstance() instead.")
    }
  }
  ...
  add(task) {
    const array = Array.from(this.#data)
    const taskExists = array.filter((t) => t.equals(task)).length > 0
    if (!taskExists) {
      this.#data.add(task)
      this.notify()
    }
  ...
  delete(taskId) {
    const array = Array.from(this.#data)
    const taskToDelete = array.find((t) => t.id === taskId)
    if (taskToDelete) {
      this.#data.delete(taskToDelete)
      this.notify()
    }
  }
...
// Apply Observer mixin
Object.assign(TaskList.prototype, observerMixin)
